...
var ETNGpager = function (srcName, dstName, cntPP, cntPS) {
    ...
    this.curP = 1;//default page 1
    this.cntPP = cntPP || 2;//default 2 record per page
    this.cntPS = cntPS || 3;//default 2 paging context per page
    this.items = [];
    this.showPNP = true;//links to last and next page
    this.showType = true;//slide to paging
    this.result = { pagedata: [], pagebar: '', limit: [0, 0], report: '' };
    this.parse();//all records
}
ETNGpager.prototype.page = function () {
    this.cntP = Math.ceil(this.cntR / this.cntPP);//all pages
    this.cntS = Math.ceil(this.cntP / this.cntPS);//all paragraphs
    this.curS = Math.ceil(this.curP / this.cntPS);//current paragraph
    this.preP = this.curP - 1;//last page
    this.nextP = this.curP + 1;//next page
    this.preS = this.curS - 1;//last paragraph
    this.nextS = this.curS + 1;//next paragraph
    this.startR = (this.curP - 1) * this.cntPP + 1;//starting record
    this.endR = (this.curP * this.cntPP > this.cntR) ? this.cntR : this.curP * this.cntPP;//ending record
